...
- tabela `events` (`id`, `data` jsonb, `owner_id` uuid, `created_at`, `allow_member_edit` boolean) — o roteiro publicado. SELECT e UPDATE só pra `authenticated`, dono OU (se `allow_member_edit = true`) quem tiver linha em `event_members` pra esse evento — **sem policy pra `anon`**. A rota pública da checklist (`GET /api/events/:id`, sem login) lê com a `SUPABASE_SERVICE_ROLE_KEY` no servidor em vez de depender de RLS aberta; ver nota de segurança abaixo. INSERT só do dono.
