...
4. **Revise na prévia**: ajuste título, textos, ícones e formato de qualquer campo; reordene ou remova cenas e fases (setas ↑/↓ e ✕); adicione cena/fase/categoria de missão manualmente; reatribua uma cena pra outra fase pelo seletor "Fase". Os itens de missão têm id estável por baixo dos panos — reordenar ou corrigir o texto de um item na caixa "Itens (um por linha)" não desalinha o progresso já marcado em campo (só editar o texto de um item específico conta como "novo item" pra fins de progresso, já que não tem outro jeito de saber que é o mesmo a partir só do texto). Preencher **início, término e local** (todos opcionais) libera o botão de Google Calendar depois de publicar — sem o término preenchido, ou se o término for antes do início, o evento é criado com 4h de duração por padrão (um aviso aparece na prévia se o término preenchido for inválido). Tem também um campo de **"Convidados do Google Calendar"** (um email por linha, opcional) — só funciona se você tiver conectado o Google, e manda um convite de verdade por email pra cada pessoa listada quando você publicar/salvar. E um campo de **"Estrutura de pastas pro Google Drive"**, pré-preenchido com o nome das fases — edite como quiser e clique em "📁 Criar estrutura no Google Drive" (exige ter conectado sua conta do Google, veja a seção própria) pra criar de verdade uma pasta com o nome do evento e as subpastas listadas, na sua conta do Drive. Use `/` numa linha pra aninhar (ex: `Final/Fotos finais` cria "Fotos finais" dentro de "Final") — pastas repetidas em mais de uma linha não duplicam, só reaproveitam a mesma.
...
6. Use a checklist em campo: marque cada cena como **Não iniciado / Em andamento / Feito / Postado**, acompanhe o progresso geral e por fase (**"Feito" e "Postado" contam igual pro percentual** de cenas capturadas — "Postado" é um passo a mais depois de capturada, pra saber quando ela foi ao ar), e marque as "missões" (momentos soltos sem ordem fixa) quando flagrar. Cada mudança de status guarda o próprio horário (ex: "Iniciado 14:02 · Concluído 14:15 · Postado 14:38") — útil pra cruzar com o horário dos arquivos na galeria depois, e o relatório calcula sozinho quanto tempo passou entre "Feito" e "Postado" em cada cena, pra apontar se o gargalo é editar ou publicar. A tela rola livremente (o topo não fica fixo), pra não ocupar espaço da tela em celular.
...
lib/pure.js             funções sem efeito colateral (dobra/compactação de progresso, validação de payload, filtro de
                        email, rate limiter) — separadas do server.js só pra dar pra testar sem subir servidor nem banco
...
scripts/compact-progress-log.js  manutenção manual do log de progresso (ver seção do event_progress acima)
...
- tabela `event_progress` (`id`, `event_id`, `action`, `payload` jsonb, `created_at`) — log append-only de cada mudança de status de cena (`action: 'status'`, com o status e os horários de "em andamento"/"feito")/missão/reset, usado pra persistir e sincronizar o progresso entre dispositivos. Linhas antigas (`action: 'record'/'unrecord'`, de antes do status em 3 níveis) continuam sendo lidas normalmente, como "feito". Continua público (sem login), de propósito. Cresce sem limite — `scripts/compact-progress-log.js` reduz o histórico de cada evento pro conjunto mínimo de linhas que reproduz o mesmo estado atual (nunca apaga estado visível, só o histórico intermediário); não é automático, roda manualmente quando fizer sentido (`node scripts/compact-progress-log.js` pra ver o que mudaria, `--apply` pra aplicar de verdade — precisa de `DATABASE_URL`, igual os testes).
